...
def load_dataset(dataset_path, n_train_data):
    ...
    """
    dataset = []
    label_dict = {'Iris-set': 0, 'Iris-versicolor': 1, 'Iris-vir': 2}
...
'''
函数sigmoid是Sigmoid激活函数的实现，
将z作为激活函数的输入，计算得到当前层的输出，并传递到下一层。
'''
def sigmoid(z):   
    """
    激活函数sigmoid
...
'''
函数sigmoid_derivative是Sigmoid函数求导的实现，
在误差反向传播的时候需要用到.
'''
...
if __name__ == "__main__":
    file_path = 'D:/Github/BP-algorithm-pratice/IrisDataset/iris.csv'
